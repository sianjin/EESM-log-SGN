...
for ichan = 1:numel(uniqueChannels)
    ...
    for inumtxrx = 1:size(uniqueNumTxRx,1)
        ...
        title(axper,['RBIR Abstracted vs Full PHY, ' num2str(uniqueNumTxRx(inumtxrx,1)) 'x' num2str(uniqueNumTxRx(inumtxrx,2)) ', ' char(uniqueChannels(ichan))]);
        grid(axper,'on')
        xlabel(axper,'RX SNR (dB)');
        ylabel(axper,'Average PER');
        ...
        ha = ha(huse);
        hi = hi(huse);
        legend([ha; hi],[arrayfun(@(x) ['RBIR (without tuning), MCS ' num2str(x)],uniqueMCS(huse),'UniformOutput',false) arrayfun(@(x) ['Full PHY, MCS ' num2str(x)],uniqueMCS(huse),'UniformOutput',false)], ...
            'location', 'best')
    end
end
