function [binsnr,binper,lut,snrEff] = awgnPerSnrFitting(abstraction,sinrStore,perStore,format,mcs,channelCoding,dataLength,rbirParameters,numSnr)

alpha = rbirParameters(1);
beta = rbirParameters(2);

% 1-D effective SNR vector: (maxNumPackets*numSnr)-by-1
snrEff = effectiveSinrVec(abstraction,sinrStore,format,mcs,alpha,beta);
