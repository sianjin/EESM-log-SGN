classdef tgaxRBIRLinkPerformanceModel < handle
...
methods
    function obj = tgaxRBIRLinkPerformanceModel(~)
        ...
        awgnlut = load('tgax_mw_awgn.mat');
        obj.awgn = awgnlut;
    end

    function per = estimateLinkPerformance(obj,snreff,varargin)
        % PER = estimateLinkPerformance(OBJ,SINR,DATALENGTH,FORMAT,MCS,CODING)
        % returns the estimated packet error rate.
        %
        %
        % DATALENGTH is the payload length in bytes.
        % 
        % FORMAT is one of 'NonHT','HTMixed','VHT','HE_SU','HE_EXT_SU','HE_MU'.
        %
    ...
    function snreff = rbirEffectiveSINR(obj,sinr,format,mcs,alpha,beta)
        % SNREFF = effectiveSINR(OBJ,SINRSTORE,FORMAT,MCS,ALPHA,BETA) returns
...
function validateMCS(mcs)
    assert(mcs>=0 && mcs<=9,'Only MCS 0-9 (BPSK, QPSK, 16QAM, and 256QAM) supported')
end
