...
% 2) Multiple interferers case shown in Fig.5 in the TCOM paper
...
intPathlossdB = 16; % Interference path loss in dB scale
...
simParams = getBox0SimParams(chan,numTxRx,mcs,cfgHE,maxnumberrors,maxNumPackets,...
    userIdx,ruIdx,Nsts,intPathlossdB);
snrs = [simParams.SNR];
...
tEnd = toc(tStart);
fname_I = sprintf('sinrPer_Config%d_%s_%s-by-%s_MCS%s.mat',cfgHE.AllocationIndex,char(chan),num2str(numTxRx(1)),num2str(numTxRx(2)),num2str(mcs));
