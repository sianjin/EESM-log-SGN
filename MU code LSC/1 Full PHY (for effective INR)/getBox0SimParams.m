...
% These arrays define the value and order INRs are defined
...
assert(all(numel(channelConfigs)==numel(inr)))
% There must be a SNR cell element for each MIMO configuration
assert(all(size(anteannaSNRConfigs,1)==cellfun(@(x)size(x,1),inr)))
for ichan = 1:numel(chans)
    ...
    for itxrx = 1:size(numTxRx,1)
        ...
        for imcs = 1:numel(mcs)
            snrIdx = mcs(imcs)+1;
            for isnr = 1:numel([inr{channelIdx}{numTxRxIdx,snrIdx}])
                ...
                sp.SNR = inr{channelIdx}{numTxRxIdx,snrIdx}(isnr);
