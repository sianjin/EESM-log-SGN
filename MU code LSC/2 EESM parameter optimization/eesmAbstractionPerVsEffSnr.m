...
betaOptSig = fminsearch(mse,beta); % Optimized EESM parameter
[binsnr,binper,lut,snrEff] = awgnPerSnrFitting(abstraction,sinrStore,perStore,format,mcs,channelCoding,dataLength,betaOptSig,numSnr);
...
save(fname_I,'snrEff','betaOptSig','allocationIndex','mcs','numTxRx','chan','maxNumPackets','snrs')
...
title(axeff,['Simulated Instantaneous PER vs Effective SNR, ' num2str(numTxRx(1)) 'x' num2str(numTxRx(2)) ', ' char(chan)]);
grid(axeff,'on')
